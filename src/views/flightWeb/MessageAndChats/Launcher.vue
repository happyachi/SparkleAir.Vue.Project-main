<template>
  <div>
    ...
    <UsersWindow
      ...
      @remove="$emit('remove', $event)"
    >
      <template v-slot:header> <slot name="header"> </slot>用戶清單 </template>
      <template v-slot:user-avatar="scopedProps">
        <slot
          name="user-avatar"
          :user="scopedProps.user"
...
import UsersWindow from "./UsersWindow.vue";
...
export default {
  ...
  computed: {
    chatWindowTitle() {
      ...
      if (
        this.chatStore.customerName !== undefined ||
        this.chatStore.customerName !== null ||
        this.chatStore.customerName !== ""
      ) {
        return this.chatStore.customerName;
      } else {
        return "";
      }
