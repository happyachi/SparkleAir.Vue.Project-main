...
const selectCouponHandler = () => {
  // 这里假设你不需要传递任何参数到 selectCoupon 方法，或者如果需要，你可以在这里添加逻辑来处理它
  // couponStore.selectCoupon(coupon); // 如果你还需要执行其他逻辑，可以保留这行代码
  console.log("123");
  window.location.href = "http://localhost:8888/FlightSearchTest";
};
</script>
    ...
    <div class="coupons-container">
      <div class="card" v-for="coupon in coupons" :key="coupon.id">
        <div class="main">
          ...
          <button @click="selectCouponHandler" class="claim-button">即刻出發</button>
        </div>
      </div>
    </div>
  </div>
